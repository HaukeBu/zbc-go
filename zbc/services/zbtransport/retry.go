...
type Operation func() (*zbdispatch.Message, error)

// MessageRetry will try to execute operation and handle retrying under specified deadline.
func MessageRetry(op Operation) (*zbdispatch.Message, error) {
	b := &backoff{
		Min:    zbcommon.BackoffMin,
		Max:    zbcommon.BackoffMax,
		Factor: 2,
		Jitter: true,
	}
	...
	for {
		msg, err := op()
		if err != nil && time.Since(start) < zbcommon.BackoffDeadline {
			time.Sleep(b.Duration())
			continue
		}
		if time.Since(start) > zbcommon.BackoffDeadline {
			return nil, zbcommon.RetryDeadlineReached
		}
